...
# fromHDF5_to_cube.py)
fileDirMHD_arr = ['MHD_beta1','MHD_beta10','MHD_beta100']
field_list = ['density','magnetic_energy_density','alfven_speed']
...
for fileDir in fileDirMHD_arr:
    ...
    for field in field_list:
        ...
        np.save('Files/'+fileDir + f"/data_{sim_name}_{field}.npy", full_x)

        # Save the following for sklearn and PyTorch to do
"""
    # split into training and test sets, save as .npy files
    train_x = full_x[0:96, :, :]
    np.save(f"train_{sim_name}_{field}.npy", train_x)

    test_x = full_x[(128 - 16 - 8):-8, :, :]
    np.save(fileDir + f"test_{sim_name}_{field}.npy", test_x)
"""
